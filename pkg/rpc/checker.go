...
func (c *checker) checkRequired(param string, value interface{}) error {
	if len(c.required) > 0 && stringutil.StringIn(param, c.required) {
		...
		case string:
			if v == "" {
				return grpcerr.New(c.ctx, grpcerr.InvalidArgument, grpcerr.ErrorMissingParameter, param)
			}
		...
		if chosen, ok := c.stringChosen[param]; ok {
			...
			case string:
				if !stringutil.StringIn(v, chosen) {
					return grpcerr.New(c.ctx, grpcerr.InvalidArgument, grpcerr.ErrorUnsupportedParameterValue, param, v)
				}
			case *wrappers.StringValue:
				if v != nil {
					if !stringutil.StringIn(v.GetValue(), chosen) {
						return grpcerr.New(c.ctx, grpcerr.InvalidArgument, grpcerr.ErrorUnsupportedParameterValue, param, v.GetValue())
					}
				}
			case []string:
				for _, s := range v {
					if !stringutil.StringIn(s, chosen) {
						return grpcerr.New(c.ctx, grpcerr.InvalidArgument, grpcerr.ErrorUnsupportedParameterValue, param, s)
					}
				}
			}
		}
