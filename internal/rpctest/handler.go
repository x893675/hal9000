...
func (p *Server) GetUser(ctx context.Context, req *pb.GetUserRequest) (*pb.User, error) {
	logger.Info(nil, "in GetUser, user is is %v", req.UserId)

	return &pb.User{

		UserId:      pbutil.ToProtoString(req.UserId),
		Username:    nil,
		Email:       nil,
		PhoneNumber: nil,
		Description: pbutil.ToProtoString("description"),
